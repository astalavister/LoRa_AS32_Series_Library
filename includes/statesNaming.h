...
typedef enum RESPONSE_STATUS {
  SUCCESS = 1,
  ERR_AS32_UNKNOWN,	/* something shouldn't happened */
  ...
  ERR_AS32_NO_RESPONSE_FROM_DEVICE,
  ERR_AS32_WRONG_UART_CONFIG,
  ERR_AS32_PACKET_TOO_BIG
} Status;
...
static String getResponseDescriptionByParams(byte status){
	switch (status)
	{
	  case SUCCESS:
		return F("Success");
		break;
	  case ERR_AS32_UNKNOWN:
		return F("Unknown");
		break;
	  case ERR_AS32_NOT_SUPPORT:
		return F("Not support!");
		break;
	  case ERR_AS32_NOT_IMPLEMENT:
		return F("Not implement");
		break;
	  case ERR_AS32_NOT_INITIAL:
		return F("Not initial!");
		break;
	  case ERR_AS32_INVALID_PARAM:
		return F("Invalid param!");
		break;
	  case ERR_AS32_DATA_SIZE_NOT_MATCH:
		return F("Data size not match!");
		break;
	  case ERR_AS32_BUF_TOO_SMALL:
		return F("Buff too small!");
		break;
	  case ERR_AS32_TIMEOUT:
		return F("Timeout!!");
		break;
	  case ERR_AS32_HARDWARE:
		return F("Hardware error!");
		break;
	  case ERR_AS32_HEAD_NOT_RECOGNIZED:
		return F("Save mode returned not recognized!");
		break;
	  case ERR_AS32_NO_RESPONSE_FROM_DEVICE:
		return F("No response from device! (Check wiring)");
		break;
	  case ERR_AS32_WRONG_UART_CONFIG:
		return F("Wrong UART configuration! (BPS must be 9600 for configuration)");
		break;
	  case ERR_AS32_PACKET_TOO_BIG:
